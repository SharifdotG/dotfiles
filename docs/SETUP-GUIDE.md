...
# GitHub CLI. Choose HTTPS when it asks for the git protocol.
gh auth login
```

> **No SSH key.** Every remote here is an HTTPS URL and authenticates with a Personal Access
> Token, on github.com and on the private forge alike. `ssh-keygen` is not part of this setup —
> a key generated by reflex just sits on disk unadded to any host. `secrets-setup.sh --ssh`
> makes one the day a host actually requires it.
...
Every remote here is **HTTPS** — what you get cloning from a browser URL — so every remote
asks again on every fetch and push. There is no SSH key quietly covering some of them.

...
| **Plasma's Fonts settings module** (`kcm_fonts`) | **`~/.config/fontconfig/fonts.conf` — a file chezmoi *does* manage** | Nothing prevents it. It re-serialises the file and **appends** its rasterisation block instead of replacing one, so the blocks accumulate and the last one wins. Found 2026-09-05 with **78 blocks** where the repo writes 1, and the machine rendering with hinting and subpixel antialiasing **off** while every config file said `hintslight` + `rgb`. Found again 2026-09-06 at **125 blocks** — it keeps growing, so treat "did you open System Settings → Fonts" as the first question when text looks wrong. `chezmoi apply` restores it; `scripts/doctor.sh` reads the *effective* values back with `fc-match` so the drift cannot be silent again, and now also runs `xmllint` over the file first — a malformed comment makes fontconfig discard the **whole** config in silence |
| **Brave** | `~/.config/BraveSoftware/Brave-Origin/Default/Preferences` | Not managed as a file — it is ~200 KB of browser-owned session state. `run_after_50-brave-fonts.sh` merges the four `webkit.webprefs.fonts.*` keys into it, because Chromium does **not** ask fontconfig for the CSS generics and an unset pref silently means Times New Roman. It refuses to write while Brave is running (the write would be discarded at exit), which is why it is `run_after_` and not `run_onchange_` — see the Fonts section of the setup guide |
